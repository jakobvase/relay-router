 ...
 * that entry.
 */
export const RouteRenderer = (props: {
  children: (props: {
    pending: boolean;
  ...
  const reversedItems = routeEntry.entries.concat([]).reverse(); // reverse is in place, but we want a copy so concat
  ...
  return <Wrapper pending={isPending}>{routeComponent}</Wrapper>;
};
